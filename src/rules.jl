...
- `patlength::Int = 0` for strings, this should usually be set to `length(pattern)`, and for regex, this means the last index of found pattern must not be less than `patlength`
